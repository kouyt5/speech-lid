...
class RML16Dataset(Dataset):
    def __init__(self, data: dict = None, key_mapping: dict = None, aug:bool = False) -> None:
        """_summary_
    ...
    def __getitem__(self, index):
        ...
        data = (data - torch.mean(data))/(torch.std(data)+1e-9)
        ...
        if self.aug:
            ...
            data[:, point:point+mask_len] = torch.randn((mask_len,))
        return key, snr, data

    def collate_fn(self, batch):
        data = torch.cat([batch[i][2].unsqueeze(0) for i in range(len(batch))], dim=0)  # N * 2 * 128
        key = torch.LongTensor([self.key_mapping[batch[i][0]] for i in range(len(batch))])  # N
        snrs = torch.LongTensor([self.snr2index(batch[i][1]) for i in range(len(batch))])
        
        index = torch.randperm(len(batch))
        data2 = torch.cat([batch[index[i].item()][2].unsqueeze(0) for i in range(len(batch))], dim=0)  # N * 2 * 128
        key2 = torch.LongTensor([self.key_mapping[batch[index[i].item()][0]] for i in range(len(batch))])  # N
        snrs2 = torch.LongTensor([self.snr2index(batch[index[i].item()][1]) for i in range(len(batch))])
...
if __name__ == "__main__":
    ...
    dataset = RML16Dataset(data_manager.get_data("train"), data_manager.get_key_mapping())

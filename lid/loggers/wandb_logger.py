...
class WandbLogger(BaseLogger):
    def __init__(self, project:str = None,  # 项目名，例如asr
        ...
        if wandb_id is not None:
            wandb.init(project=project, entity=entity, name=name, resume="allow", save_code=True, id=wandb_id, group=group)
            logging.info(f"wandb 初始化完成: checkpoint="+str(self.checkpint_path))
        else:
            wandb.init(project=project, entity=entity, name=name, save_code=True, group=group)  # 新建一个wandb

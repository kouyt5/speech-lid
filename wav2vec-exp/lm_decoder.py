...
if __name__ == "__main__":
    vocab = [
        c.strip()
        for c in open(
            "/home/cc/workdir/code/wav2vec-exp/data/vocab_A.txt", "r"
        ).readlines()
    ]
    lm_path = "/home/cc/workdir/code/wav2vec-exp/ckpts/3-gram.pruned.3e-7.arpa"
    decoder = BeamSearchDecoderWithLM(
        vocab=vocab,
        beam_width=500,
        alpha=2.0,
        beta=-1.0,
    ...
    log_prob = torch.randn((2, 1000, 29), dtype=torch.float32)
    # log_prob = torch.log_softmax(log_prob, dim=-1).numpy()
    log_prob = torch.softmax(log_prob, dim=-1).numpy()

...
class CTCTokenizer:
    ...
    def parallel_ctc_prefix_search(
        self,
        predictions: torch.Tensor,
        predictions_lengths: torch.Tensor,
        beam_size: int,
    ):
        beam_size = min(len(self.export_vocab())+1, beam_size)
        order_tensor_list = [
            predictions[i][: predictions_lengths[i].item()].detach().cpu()
            for i in range(predictions.shape[0])
        ]
        ctc_prefix_func = partial(self._ctc_prefix_beam_search, beam_size=beam_size)
        iter = self.pool.imap(ctc_prefix_func, order_tensor_list)
        result = []
        for item in iter:
    ...
    def encoder(self, s: str) -> torch.Tensor:
        ...
        for c in s:
            if c not in self.s2labels_map.keys():
                logging.warning(f"char: {c} is not in dict and will be replace as _")

...
URLS = {
    ...
    'LIBRISPEECH_FINETUNING': "https://dl.fbaipublicfiles.com/librilight/data/librispeech_finetuning.tgz"
}
...
def __process_lirbi_light_10(data_folder: str, dst_folder: str, num_workers=1):
    """单独处理libri_light 10子集

    Args:
        data_folder (str): 'librispeech_finetuning' below is (1h, 9h, phones)
        dst_folder (str): 'librispeech_finetuning-processed' below is (1h, 9h)
        num_workers (int, optional): thread number. Defaults to 1.
    """
    
    if not os.path.exists(dst_folder):
        os.makedirs(dst_folder)

    # ************************stage 1 find wav folder and trans.txt pair*****************************
    # 1h
    one_h_clean_datas = []  # (path/to/wav/, path/to/wav/*.trans.txt)
    one_h_other_datas = []
    one_hour_data_folder = os.path.join(data_folder, "1h")  # 0, 1,2,3...
    for i in ["0","1", "2", "3", "4", "5"]:
        clean_other_folder = os.path.join(one_hour_data_folder, i)  # clean other
        # clean
        for root, dirnames, filenames in os.walk(os.path.join(clean_other_folder, "clean")):
            for filename in fnmatch.filter(filenames, '*.trans.txt'):
                one_h_clean_datas.append((os.path.join(root, filename), root))
        # other
        for root, dirnames, filenames in os.walk(os.path.join(clean_other_folder, "other")):
            for filename in fnmatch.filter(filenames, '*.trans.txt'):
                one_h_other_datas.append((os.path.join(root, filename), root))
        
    # 9h
    nine_h_clean_datas = []  # (path/to/wav/, path/to/wav/*.trans.txt)
    nine_h_other_datas = []
    nine_hour_data_folder = os.path.join(data_folder, "9h")  # 0, 1,2,3...
    # clean 9h
    for root, dirnames, filenames in os.walk(os.path.join(nine_hour_data_folder, "clean")):
        for filename in fnmatch.filter(filenames, '*.trans.txt'):
            nine_h_clean_datas.append((os.path.join(root, filename), root))
    # other 9h
    for root, dirnames, filenames in os.walk(os.path.join(nine_hour_data_folder, "other")):
        for filename in fnmatch.filter(filenames, '*.trans.txt'):
            nine_h_other_datas.append((os.path.join(root, filename), root))
    
    
    # ************************stage 2 find every pair to process*****************************
    # total generate 4 json file
    one_clean_entitys = []
    one_other_entitys = []
    nine_clean_entitys = []
    nine_other_entitys = []
    for datas,entries, hours in zip([one_h_clean_datas, one_h_other_datas, nine_h_clean_datas, nine_h_other_datas],
                              [one_clean_entitys, one_other_entitys, nine_clean_entitys, nine_other_entitys],
                              ['1h-clean', '1h-other', '9h-clean', '9h-other']):
        for transcripts_file, root in tqdm(datas, total=len(datas), desc="读取音频路径和文本"):
            with open(transcripts_file, encoding="utf-8") as fin:
                for line in fin:
                    id, text = line[: line.index(" ")], line[line.index(" ") + 1 :]
                    transcript_text = text.lower().strip()
        ...
        curr_dst_folder = os.path.join(dst_folder, hours)
        if not os.path.exists(curr_dst_folder):
            os.mkdir(curr_dst_folder)
        manifest_file = os.path.join(dst_folder, hours+".json")
        target_lists = transform_all_wavs(entries, target_wav_dir=curr_dst_folder, num_workers=num_workers)
        with open(manifest_file, 'w', encoding='utf-8') as f:
            for json_line in target_lists:
                f.write(json.dumps(json_line, ensure_ascii=False) + '\n')
            logging.info("写入json文件完成: " + manifest_file)
...
def main():
    ...
    for data_set in data_sets.split(','):
        ...
        if data_sets == "librispeech_finetuning":
            filepath = os.path.join(data_root, data_set + ".tgz")
        ...
        if data_sets == "librispeech_finetuning":
            __process_lirbi_light_10(
                os.path.join(data_root, "librispeech_finetuning"),  # 1h, 9h, phones
                os.path.join(data_root, "librispeech_finetuning") + "-processed",  # 1h, 9h
